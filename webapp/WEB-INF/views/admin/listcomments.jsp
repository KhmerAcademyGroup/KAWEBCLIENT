<%@page import="model.dto.Comment"%>
<%@ page language="java" contentType="text/html; charset=utf-8"
    pageEncoding="utf-8"%>
    
<%@page import="java.util.ArrayList"%>    
<%
	ArrayList list = (ArrayList)request.getAttribute("listcomment");
%>
<!DOCTYPE html>
<html lang="en">
	<head>
		...
		<title>Comments List | Khmer Academy</title>
						...
						<table class="table table-striped table-hover" id="datatable-example">
							...
							<tbody>
								...
								<%for(int i=0;i<list.size();i++){
									Comment cmt = (Comment)list.get(i);		
								%>
								<tr>
									<td><%=cmt.getCommentid() %></td>
									<td><%=cmt.getCommenttext() %></td>
									<td><%=cmt.getCommentdate() %></td>
									<td><%=cmt.getVideoid() %></td>
									<td><%=cmt.getUserid() %></td>
									
									<td>
										<a style="margin-right:3px" href="../elearning/play.act?v=<%=cmt.getVideoid() %>" target="_blank" class="btn btn-danger">View</a>
